...
TBC... this works for the example, but not for the full input.
...
addnode <- function(r, c) {
  if (between(r, 1, nrow(antinodes)) && between(c, 1, ncol(antinodes))) {
    epsilon <- abs(c - round(c))
    if (epsilon < 1e-10) {
      antinodes[r, round(c)] <<- antinodes[r, round(c)] + 1
    }
  }
}
```
...
find_antinodes <- function(p1, p2) {
  ...
  for (the_row in 1:nrow(mapmat)) {
    the_col <- predict(mod, newdat = data.frame(row = the_row))
    addnode(the_row, the_col)
  }
}
```
